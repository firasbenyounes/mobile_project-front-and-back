...
async function AddofferAction(label,image) {
        
  await fetch('http://'+global.ipAddress+':5000/api/v1/offer/'+global.currentspaceid, {
      method: 'POST',
      headers: {
...
class Newoffre extends React.Component {
  constructor(props)
  {
      super(props)
      ...
      ButtonClickListener=async()=>
      {
          ...
          etat=await AddofferAction(Title,image);
           //alert(etat)
               if(etat=="success")
               {
           ...
           this.props.navigation.navigate('ListoffresResponsable', {
            Refresh:"Yes",
          })
     ...
     <ImageBackground
      ...
      <Button onPress={this.ButtonClickListener  } color="primary" style={styles.createButton}>
        ...
        return (
          ...
          <Image source={{ uri: this.state.image }} style={{ height: 122, width: 'auto',
               } } />
